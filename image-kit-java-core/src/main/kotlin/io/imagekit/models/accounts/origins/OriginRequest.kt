...
@JsonDeserialize(using = OriginRequest.Deserializer::class)
@JsonSerialize(using = OriginRequest.Serializer::class)
class OriginRequest
private constructor(
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = originRequest.accept(new OriginRequest.Visitor<Optional<String>>() {
     *     @Override
     *     public Optional<String> visitS3(S3 s3) {
     *         return Optional.of(s3.toString());
     *     }
     *
     *     // ...
     *
     ...
     *   expected type.
     */
    fun validate(): OriginRequest = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun toString(): String =
        when {
            s3 != null -> "OriginRequest{s3=$s3}"
            s3Compatible != null -> "OriginRequest{s3Compatible=$s3Compatible}"
            cloudinaryBackup != null -> "OriginRequest{cloudinaryBackup=$cloudinaryBackup}"
            webFolder != null -> "OriginRequest{webFolder=$webFolder}"
            webProxy != null -> "OriginRequest{webProxy=$webProxy}"
            gcs != null -> "OriginRequest{gcs=$gcs}"
            azureBlob != null -> "OriginRequest{azureBlob=$azureBlob}"
            akeneoPim != null -> "OriginRequest{akeneoPim=$akeneoPim}"
            _json != null -> "OriginRequest{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid OriginRequest")
        }

    companion object {

        @JvmStatic fun ofS3(s3: S3) = OriginRequest(s3 = s3)

        @JvmStatic
        fun ofS3Compatible(s3Compatible: S3Compatible) = OriginRequest(s3Compatible = s3Compatible)

        @JvmStatic
        fun ofCloudinaryBackup(cloudinaryBackup: CloudinaryBackup) =
            OriginRequest(cloudinaryBackup = cloudinaryBackup)

        @JvmStatic fun ofWebFolder(webFolder: WebFolder) = OriginRequest(webFolder = webFolder)

        @JvmStatic fun ofWebProxy(webProxy: WebProxy) = OriginRequest(webProxy = webProxy)

        @JvmStatic fun ofGcs(gcs: Gcs) = OriginRequest(gcs = gcs)

        @JvmStatic fun ofAzureBlob(azureBlob: AzureBlob) = OriginRequest(azureBlob = azureBlob)

        @JvmStatic fun ofAkeneoPim(akeneoPim: AkeneoPim) = OriginRequest(akeneoPim = akeneoPim)
    }

    /**
     * An interface that defines how to map each variant of [OriginRequest] to a value of type [T].
     */
    interface Visitor<out T> {
        ...
        /**
         * Maps an unknown variant of [OriginRequest] to a value of type [T].
         *
         * An instance of [OriginRequest] can contain an unknown variant if it was deserialized from
        ...
        fun unknown(json: JsonValue?): T {
            throw ImageKitInvalidDataException("Unknown OriginRequest: $json")
        }
    }

    internal class Deserializer : BaseDeserializer<OriginRequest>(OriginRequest::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): OriginRequest {
            ...
            when (type) {
                "S3" -> {
                    return tryDeserialize(node, jacksonTypeRef<S3>())?.let {
                        OriginRequest(s3 = it, _json = json)
                    } ?: OriginRequest(_json = json)
                }
                "S3_COMPATIBLE" -> {
                    return tryDeserialize(node, jacksonTypeRef<S3Compatible>())?.let {
                        OriginRequest(s3Compatible = it, _json = json)
                    } ?: OriginRequest(_json = json)
                }
                "CLOUDINARY_BACKUP" -> {
                    return tryDeserialize(node, jacksonTypeRef<CloudinaryBackup>())?.let {
                        OriginRequest(cloudinaryBackup = it, _json = json)
                    } ?: OriginRequest(_json = json)
                }
                "WEB_FOLDER" -> {
                    return tryDeserialize(node, jacksonTypeRef<WebFolder>())?.let {
                        OriginRequest(webFolder = it, _json = json)
                    } ?: OriginRequest(_json = json)
                }
                "WEB_PROXY" -> {
                    return tryDeserialize(node, jacksonTypeRef<WebProxy>())?.let {
                        OriginRequest(webProxy = it, _json = json)
                    } ?: OriginRequest(_json = json)
                }
                "GCS" -> {
                    return tryDeserialize(node, jacksonTypeRef<Gcs>())?.let {
                        OriginRequest(gcs = it, _json = json)
                    } ?: OriginRequest(_json = json)
                }
                "AZURE_BLOB" -> {
                    return tryDeserialize(node, jacksonTypeRef<AzureBlob>())?.let {
                        OriginRequest(azureBlob = it, _json = json)
                    } ?: OriginRequest(_json = json)
                }
                "AKENEO_PIM" -> {
                    return tryDeserialize(node, jacksonTypeRef<AkeneoPim>())?.let {
                        OriginRequest(akeneoPim = it, _json = json)
                    } ?: OriginRequest(_json = json)
                }
            }

            return OriginRequest(_json = json)
        }
    }

    internal class Serializer : BaseSerializer<OriginRequest>(OriginRequest::class) {
