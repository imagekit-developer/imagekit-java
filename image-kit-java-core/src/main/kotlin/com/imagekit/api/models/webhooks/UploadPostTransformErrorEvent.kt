...
private constructor(
     ...
     * ```java
     * JsonValue.from("upload.post-transform.error")
     * ```
     *
    ...
    class Builder internal constructor() {
        ...
        private var type: JsonValue = JsonValue.from("upload.post-transform.error")
    ...
    fun validate(): UploadPostTransformErrorEvent = apply {
        ...
        request().validate()
        _type().let {
            if (it != JsonValue.from("upload.post-transform.error")) {
    ...
    internal fun validity(): Int =
        (if (id.asKnown().isPresent) 1 else 0) +
            ...
            type.let { if (it == JsonValue.from("upload.post-transform.error")) 1 else 0 }
    ...
    private constructor(
        ...
        ) : this(fileId, name, path, transformation, url, mutableMapOf())
        ...
        private val hashCode: Int by lazy {
            Objects.hash(fileId, name, path, transformation, url, additionalProperties)
        }

        override fun hashCode(): Int = hashCode

        override fun toString() =
            "Data{fileId=$fileId, name=$name, path=$path, transformation=$transformation, url=$url, additionalProperties=$additionalProperties}"
    }
