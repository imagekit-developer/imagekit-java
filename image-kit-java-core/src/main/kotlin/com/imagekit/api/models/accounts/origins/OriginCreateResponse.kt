...
private constructor(
    ...
    override fun toString(): String =
        when {
            ...
            gcs != null -> "OriginCreateResponse{gcs=$gcs}"
            azureBlob != null -> "OriginCreateResponse{azureBlob=$azureBlob}"
    ...
    companion object {
        ...
        @JvmStatic fun ofGcs(gcs: Gcs) = OriginCreateResponse(gcs = gcs)

        @JvmStatic
        fun ofAzureBlob(azureBlob: AzureBlob) = OriginCreateResponse(azureBlob = azureBlob)
        ...
        override fun ObjectCodec.deserialize(node: JsonNode): OriginCreateResponse {
            ...
            when (type) {
                "S3" -> {
                    ...
                    } ?: OriginCreateResponse(_json = json)
                }
                "S3_COMPATIBLE" -> {
                ...
                "GCS" -> {
                    return tryDeserialize(node, jacksonTypeRef<Gcs>())?.let {
                        OriginCreateResponse(gcs = it, _json = json)
                    } ?: OriginCreateResponse(_json = json)
                }
                "AZURE_BLOB" -> {
                    return tryDeserialize(node, jacksonTypeRef<AzureBlob>())?.let {
                        OriginCreateResponse(azureBlob = it, _json = json)
                    } ?: OriginCreateResponse(_json = json)
                }
                "AKENEO_PIM" -> {
            ...
            return OriginCreateResponse(_json = json)
        }
    }
