...
@JsonDeserialize(using = OriginCreateResponse.Deserializer::class)
@JsonSerialize(using = OriginCreateResponse.Serializer::class)
class OriginCreateResponse
private constructor(
    ...
    fun validate(): OriginCreateResponse = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is OriginCreateResponse &&
            s3 == other.s3 &&
    ...
    override fun toString(): String =
        when {
            s3 != null -> "OriginCreateResponse{s3=$s3}"
            s3Compatible != null -> "OriginCreateResponse{s3Compatible=$s3Compatible}"
            cloudinaryBackup != null -> "OriginCreateResponse{cloudinaryBackup=$cloudinaryBackup}"
            webFolder != null -> "OriginCreateResponse{webFolder=$webFolder}"
            webProxy != null -> "OriginCreateResponse{webProxy=$webProxy}"
            googleCloudStorageGcs != null ->
                "OriginCreateResponse{googleCloudStorageGcs=$googleCloudStorageGcs}"
            azureBlobStorage != null -> "OriginCreateResponse{azureBlobStorage=$azureBlobStorage}"
            akeneoPim != null -> "OriginCreateResponse{akeneoPim=$akeneoPim}"
            _json != null -> "OriginCreateResponse{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid OriginCreateResponse")
        }

    companion object {

        @JvmStatic fun ofS3(s3: S3) = OriginCreateResponse(s3 = s3)

        @JvmStatic
        fun ofS3Compatible(s3Compatible: S3Compatible) =
            OriginCreateResponse(s3Compatible = s3Compatible)

        @JvmStatic
        fun ofCloudinaryBackup(cloudinaryBackup: CloudinaryBackup) =
            OriginCreateResponse(cloudinaryBackup = cloudinaryBackup)

        @JvmStatic
        fun ofWebFolder(webFolder: WebFolder) = OriginCreateResponse(webFolder = webFolder)

        @JvmStatic fun ofWebProxy(webProxy: WebProxy) = OriginCreateResponse(webProxy = webProxy)
        ...
        fun ofAzureBlobStorage(azureBlobStorage: AzureBlobStorage) =
            OriginCreateResponse(azureBlobStorage = azureBlobStorage)

        @JvmStatic
        fun ofAkeneoPim(akeneoPim: AkeneoPim) = OriginCreateResponse(akeneoPim = akeneoPim)
    }

    /**
     * An interface that defines how to map each variant of [OriginCreateResponse] to a value of
     * type [T].
     */
    interface Visitor<out T> {
        ...
        /**
         * Maps an unknown variant of [OriginCreateResponse] to a value of type [T].
         *
         * An instance of [OriginCreateResponse] can contain an unknown variant if it was
        ...
        fun unknown(json: JsonValue?): T {
            throw ImageKitInvalidDataException("Unknown OriginCreateResponse: $json")
        }
    }

    internal class Deserializer :
        BaseDeserializer<OriginCreateResponse>(OriginCreateResponse::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): OriginCreateResponse {
            val json = JsonValue.fromJsonNode(node)

            val bestMatches =
                sequenceOf(
                        tryDeserialize(node, jacksonTypeRef<S3>())?.let {
                            OriginCreateResponse(s3 = it, _json = json)
                        },
                        tryDeserialize(node, jacksonTypeRef<S3Compatible>())?.let {
                            OriginCreateResponse(s3Compatible = it, _json = json)
                        },
                        tryDeserialize(node, jacksonTypeRef<CloudinaryBackup>())?.let {
                            OriginCreateResponse(cloudinaryBackup = it, _json = json)
                        },
                        tryDeserialize(node, jacksonTypeRef<WebFolder>())?.let {
                            OriginCreateResponse(webFolder = it, _json = json)
                        },
                        tryDeserialize(node, jacksonTypeRef<WebProxy>())?.let {
                            OriginCreateResponse(webProxy = it, _json = json)
                        },
                        tryDeserialize(node, jacksonTypeRef<GoogleCloudStorageGcs>())?.let {
                            OriginCreateResponse(googleCloudStorageGcs = it, _json = json)
                        },
                        tryDeserialize(node, jacksonTypeRef<AzureBlobStorage>())?.let {
                            OriginCreateResponse(azureBlobStorage = it, _json = json)
                        },
                        tryDeserialize(node, jacksonTypeRef<AkeneoPim>())?.let {
                            OriginCreateResponse(akeneoPim = it, _json = json)
                        },
                    )
                    .filterNotNull()
            ...
            return when (bestMatches.size) {
                ...
                0 -> OriginCreateResponse(_json = json)
    ...
    internal class Serializer : BaseSerializer<OriginCreateResponse>(OriginCreateResponse::class) {

        override fun serialize(
            value: OriginCreateResponse,
    ...
    private constructor(
        ...
        override fun toString() =
            "GoogleCloudStorageGcs{bucket=$bucket, clientEmail=$clientEmail, name=$name, privateKey=$privateKey, type=$type, id=$id, baseUrlForCanonicalHeader=$baseUrlForCanonicalHeader, includeCanonicalHeader=$includeCanonicalHeader, prefix=$prefix, additionalProperties=$additionalProperties}"
    }
    ...
    private constructor(
        ...
        override fun toString() =
            "AzureBlobStorage{accountName=$accountName, container=$container, name=$name, sasToken=$sasToken, type=$type, id=$id, baseUrlForCanonicalHeader=$baseUrlForCanonicalHeader, includeCanonicalHeader=$includeCanonicalHeader, prefix=$prefix, additionalProperties=$additionalProperties}"
    }
