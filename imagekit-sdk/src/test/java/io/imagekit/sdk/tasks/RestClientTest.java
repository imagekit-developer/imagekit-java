...
public class RestClientTest {
	...
	public void valid_updateDetail_expectSuccess() {
		String resp = "{\n" + "    \"fileId\" : \"598821f949c0a938d57563bd\"," + "    \"type\": \"file\","
				+ "    \"name\": \"demo.jpg\",\n" + "    \"filePath\": \"/demo.jpg\",\n"
	...
	public void internalServerError_updateDetail_expectFailed() {
		...
		OkHttpClientStub clientStub = new OkHttpClientStub(json.toString(), 500, "Internal Server Error");
		SUT.setClient(clientStub);
	...
	public void unprocessable_updateDetail_expectFailed() {
		JsonObject json = new JsonObject();
		json.addProperty("message", "Internal Server Error");
		OkHttpClientStub clientStub = new OkHttpClientStub(json.toString(), 422, "Un-processable Entity");
		SUT.setClient(clientStub);
		FileUpdateRequest fileUpdateRequest = new FileUpdateRequest("my_file_id");
		Result result = SUT.updateDetail(fileUpdateRequest);
		assertNotNull(result.getMessage());
	...
	public void unProcessableEntity_getFileList_expectFailed() {
		...
		OkHttpClientStub clientStub = new OkHttpClientStub(obj.toString(), 422, "Un-processable Entity");
		SUT.setClient(clientStub);
		Map<String, String> options = new HashMap<>();
		options.put("skip", "0");
		options.put("limit", "10");
	...
	public void internalServerError_getFileList_expectFailed() {
		...
		OkHttpClientStub clientStub = new OkHttpClientStub(obj.toString(), 500, "Internal Server Error");
		SUT.setClient(clientStub);
		Map<String, String> options = new HashMap<>();
		options.put("skip", "0");
		options.put("limit", "10");
	...
	public void getFileDetails_un_processable_entity() {
		...
		OkHttpClientStub clientStub = new OkHttpClientStub(obj.toString(), 433, "Un-processable Entity");
		SUT.setClient(clientStub);
		Result result = SUT.getFileDetail("fileId");
		assertNotNull(result.getMessage());
	}
	...
	public void getFileMetaData_valid_request_expect_success() {
		String resp = "{"
				...
				+ "}, \"gps\":{" + "\"GPSVersionID\":[" + "2, 2, 0, 0" + "]" + "}, \"interoperability\":{"
				+ "\"InteropIndex\":\"R98\", \"InteropVersion\":\"0100\"" + "}, \"makernote\":{}" + "}" + "}";

		OkHttpClientStub clientStub = new OkHttpClientStub(resp, 200, "Ok");
		SUT.setClient(clientStub);
		ResultMetaData result = SUT.getFileMetaData("598821f949c0a938d57563bd");
		assertEquals("https://api.imagekit.io/v1/files/598821f949c0a938d57563bd/metadata",
				SUT.request.url().toString());
		assertThat(resp, is(result.getRaw()));
	}
	...
	public void getRemoteFileMetaData_valid_request_expect_success() {
		...
		assertEquals("https://api.imagekit.io/v1/metadata?url=http://remote_url.example.com/demo.png",
				SUT.request.url().toString());
		assertThat(resp, is(result.getRaw()));
	}
	...
	public void deleteFile_valid_request_expect_success() {
		...
		OkHttpClientStub clientStub = new OkHttpClientStub(obj.toString(), 204, "Ok");
		SUT.setClient(clientStub);
	...
	public void bulkDeleteFiles_file_id_not_found() {
		...
		fileIds.add("561c13d0c533c3437x434409");
		fileIds.add("561c13d0c533c3437x434411");
		OkHttpClientStub clientStub = new OkHttpClientStub(resp, 404, "Not found");
		SUT.setClient(clientStub);
		ResultFileDelete result = SUT.bulkDeleteFiles(fileIds);
	...
	public void bulkDeleteFiles_internal_server_error() {
		...
		OkHttpClientStub clientStub = new OkHttpClientStub(obj.toString(), 500, "Ok");
		SUT.setClient(clientStub);
		ResultFileDelete result = SUT.bulkDeleteFiles(fileIds);
		assertThat("Error: Internal server error.", is(result.getMessage()));
	}
	...
	public void purgeCache_valid_request_expect_success() {
		...
		OkHttpClientStub clientStub = new OkHttpClientStub(obj.toString(), 200, "Ok");
		SUT.setClient(clientStub);
	...
	private static class OkHttpClientStub extends OkHttpClient {
		...
		public OkHttpClientStub(String response, int code, String message) {
			...
			builder.code(code).message(message).protocol(Protocol.HTTP_1_1)
				...
				public Response execute() throws IOException {
					...
					if (credential == null) {
						builder.body(ResponseBody.create(MediaType.parse("application/json"),
								"{\"message\":\"Your request does not contain private API key.\",\"help\":\"For support kindly contact us at support@imagekit.io .\"}"));
